...
- **Surrogate int PKs for catalogue/pricing rows, UUID PKs for user data, slug snapshots for orders.** `products`, `template_categories`, `templates` and the pricing tables use an `int` autoincrement PK plus a unique `slug` column carrying the human-readable id already used in URLs and `src/lib/templates.ts` (`order-of-service`, `silk`, ...) — small/fast to join, appropriate since these rows are only ever written by our own seed script, never an untrusted client. `users`, `designs`, `orders`, etc. use `char(36)` UUIDs (`crypto.randomUUID()`) instead, since those rows are created by real (or guest) visitors. `order_items`/`orders` spec columns go a third way: plain `varchar` **slugs with no FK at all** — see the snapshot point below for why.

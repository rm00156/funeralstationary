...
`/admin` manages the catalogue: products + their six pricing tables (`/admin/products/[slug]`), template categories, template metadata, and visual template-layout authoring (`/admin/templates/[slug]/layout` mounts `DesignEditor` in `templateAuthoring` mode — saves PUT `templates.layout` instead of `/api/designs`; it sits outside the `(protected)` route group because the editor is full-bleed, and calls `requireAdmin()` itself).
